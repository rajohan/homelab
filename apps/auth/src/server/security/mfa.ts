...
export class MultiFactor {
    ...
    /**
     * Bind second-factor workflows to the account service.
    ...
    /**
     * List the distinct enrolled second-factor kinds.
     * @param principal - The account whose methods are requested.
     * @returns The enrolled factor kinds.
     */
    ...
    /**
     * Check whether an account still has unused recovery codes.
     * @param principal - The account whose recovery inventory is inspected.
     * @returns Whether any one-use recovery code remains.
     */
    ...
    /**
     * Replace stored recovery digests and return fresh codes for one-time display.
     * @param store - The caller-owned transaction.
     * @param userId - The account receiving new recovery codes.
     * @returns The new plaintext codes; callers must deliver them privately.
     */
    ...
    /**
     * Complete MFA enrollment and provision recovery after the account's first factor.
     * @param store - The caller-owned transaction.
     * @param principal - The enrolling account/session principal.
    ...
    /**
     * Enforce the maximum enrolled-factor count before inserting a new factor.
     * @param store - The caller-owned account transaction.
     * @param userId - The account receiving another factor.
     * @returns Completion when another factor may be enrolled.
     */
    ...
    /**
     * Create a session-bound TOTP enrollment challenge after fresh identity proof.
     * @param principal - The authenticated enrolling session.
     * @param label - The user-chosen authenticator label.
     * @returns The one-use enrollment token and authenticator setup information.
     */
    ...
    /**
     * Verify the enrollment code and store the new TOTP factor atomically.
     * @param principal - The session that initiated enrollment.
     * @param token - The one-use enrollment token.
     * @param code - The current code generated by the new authenticator.
     * @returns Any recovery codes issued when first enabling MFA.
     */
    ...
    /**
     * Create a user-verified WebAuthn registration challenge bound to the session and origin.
     * @param principal - The authenticated enrolling session.
     * @param origin - The validated relying-party origin.
     * @returns The registration options and one-use challenge token.
     */
    ...
    /**
     * Verify and persist a WebAuthn registration without weakening user verification.
     * @param principal - The session that initiated registration.
     * @param token - The session-bound enrollment token.
     * @param label - The name to display for the credential.
     * @param response - The browser's signed registration result.
     * @returns Any recovery codes issued when first enabling MFA.
     */
    ...
    /**
     * Prepare a user-verified assertion with enrolled credentials and their transport hints.
     * @param principal - The session requiring a second-factor proof.
     * @param origin - The validated relying-party origin.
     * @returns The assertion options and session-bound challenge token.
     */
    ...
    /**
     * Verify a WebAuthn assertion and update replay counters and session assurance atomically.
     * @param principal - The session completing the proof.
     * @param token - The one-use assertion challenge token.
     * @param response - The browser's signed assertion result.
     * @returns Completion after the assertion is accepted.
     */
    async webauthnProof(
        principal: Principal,
        token: string,
    ...
    /**
     * Verify a TOTP or consume one recovery code and complete session MFA.
     * @param principal - The session completing the proof.
     * @param code - The submitted authenticator or recovery code.
     * @param recovery - Whether to interpret the proof as a recovery code.
     * @returns Completion after the proof is accepted atomically.
     */
    async codeProof(
        principal: Principal,
        code: string,
    ...
    /**
     * Remove an owned factor and revoke dependent assurance when the last factor is removed.
     * @param principal - The session authorizing the protected removal.
     * @param factorId - The owned factor identifier.
     * @returns Completion after factor and session changes commit.
     */
