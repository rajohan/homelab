...
import { createAuthApplication } from "../apps/auth/src/application";
import type { AuthConfiguration } from "../apps/auth/src/configuration";
import {
    users,
    sessions,
    mailOutbox,
    rateBuckets,
    oidcRecords,
} from "../apps/auth/src/database/schema";
import { hashPassword } from "../apps/auth/src/security/crypto";
    ...
    for (const header of response.headers.getSetCookie()) {
        ...
        const path =
            attributes
                ...
                ?.trim()
                .slice(5) ?? "/";
        cookies.set(pair.slice(0, separator), { value: pair.slice(separator + 1), path });
    }
    return response;
}
beforeAll(async () => {
    ...
    authServer = Bun.serve({
        ...
        port: 0,
        fetch: (request) =>
            auth ? auth.handle(request, "bff-test") : new Response(null, { status: 503 }),
    });
    ...
    await migrate(auth.connection.database, {
        migrationsFolder: new URL("../apps/auth/migrations/", import.meta.url).pathname,
    });
...
describe("independent dashboard BFF and identity service", () => {
    ...
    test("uses server-side step-up and rejects a forged dashboard origin", async () => {
        ...
        const failure = v.parse(v.object({ code: v.string() }), await rejected.json());
        expect(failure.code).toBe("STEP_UP_REQUIRED");
        const proof = await browser(`${origin}/api/account/proof/password`, { password });
    ...
    test("rejects a tampered dashboard session instead of exposing account data", async () => {
        jar.get(origin)?.set("homelab_dashboard", { value: "tampered", path: "/" });
